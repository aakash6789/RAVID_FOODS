        ...
        <div className="max-w-7xl mx-auto px-10 py-12">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-8 gap-y-10">
            {data.map((cat, idx) => {
              ...
              return (
                <div key={idx} className="group">
                  {/* CHANGED FROM <h4> TO <NavLink> TO MAKE IT CLICKABLE */}
                  <NavLink
                    to={`${baseUrl}/${cat.slug}`}
                    onClick={() => setOpen(false)}
                    className="block text-[13px] font-bold text-gray-900 mb-4 uppercase tracking-wider border-l-2 border-orange-500 pl-3 hover:text-orange-600 transition-colors cursor-pointer"
                  >
                    {cat.title}
                  </NavLink>
